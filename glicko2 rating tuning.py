...
#skills = np.random.chisquare(15,size)
...
point_5=np.percentile(raw_users[:,1],q=[0,35,70,92,100])
point_5
...
def init_user_record():
    for i in range(1,10001):
        c.execute('''DELETE FROM user'''+str(i)+'''_record''')
        conn.commit()
...
def matchmake(match_size=2): #3000명 중 rating +-2*rd인 사용자와 매칭시켜줌 [(,),(,),..] 튜플의 리스트 반환
    matches = []
    run_count = 0
    while True:
        run_count += 1
...
def get_Sa(userA, userB):
    ...
    '''
    probA = np.random.randint(low=1,high=100,size=momentum)
    probB = []
    for i in range(momentum):
        probB.append(int(1/(1+10**((userA[1]-userB[1])/400))*100))
...
def compute_v(mu, muj_list, RDj_list):
    tempSum=0
    for i in range(len(muj_list)):
        tempE = E(mu, muj_list[i], RDj_list[i])
        tempSum += g(RDj_list[i])**2 * tempE * (1 - tempE)
    return 1 / tempSum
...
def compute_delta(mu,muj_list,RDj_list,Sj_list,v):
    tempSum=0
    for i in range(len(muj_list)):
        tempSum += g(RDj_list[i])*(Sj_list[i]-E(mu,muj_list[i],RDj_list[i]))
    return v*tempSum
...
def compute_mu_dash(mu, RD_dash, muj_list, RDj_list, Sj_list):
    tempSum=0
    for i in range(len(muj_list)):
        tempSum += g(RDj_list[i]) * (Sj_list[i] - E(mu, muj_list[i], RDj_list[i]))
    return mu + RD_dash**2 * tempSum
...
def match_glicko(userA, userB):
    ...
    c.execute("SELECT * FROM user"+str(userA[0])+"_record")
    record_A=c.fetchall()
    if len(record_A) !=0 and len(record_A)%10==0:
        muj_list_A=[]; RDj_list_A=[]; Sj_list_A=[]
        for i in range(len(record_A)):
            muj_list_A.append((record_A[i][3]-1500)/SCALE)
            RDj_list_A.append((record_A[i][4]/SCALE))
            Sj_list_A.append(record_A[i][5])
        v1 = compute_v(mu1, muj_list_A, RDj_list_A)
        delta1 = compute_delta(mu1, muj_list_A, RDj_list_A, Sj_list_A, v1)
        SIGMA1 = compute_SIGMA(delta1, RD1, SIGMA_A, v1)
        RD_dash1 = compute_RD_dash(RD1, v1, SIGMA1)
        mu_dash1 = compute_mu_dash(mu1, RD_dash1, muj_list_A, RDj_list_A, Sj_list_A)
        #print(record_A)
        c.execute("DELETE FROM user"+str(userA[0])+"_record")
        conn.commit()
        
    else: 
        SIGMA1=SIGMA_A
        mu_dash1=mu1; RD_dash1=RD1
        
    c.execute("SELECT * FROM user"+str(userB[0])+"_record")
    record_B=c.fetchall()
    if len(record_B) !=0 and len(record_B)%10==0:
        muj_list_B=[]; RDj_list_B=[]; Sj_list_B=[]
        for i in range(len(record_B)):
            muj_list_B.append((record_B[i][3]-1500)/SCALE)
            RDj_list_B.append((record_B[i][4]/SCALE))
            Sj_list_B.append(record_B[i][5])
        v2 = compute_v(mu2, muj_list_B, RDj_list_B)
        delta2 = compute_delta(mu2, muj_list_B, RDj_list_B, Sj_list_B, v2)
        SIGMA2 = compute_SIGMA(delta2, RD2, SIGMA_B, v2)
        RD_dash2 = compute_RD_dash(RD2, v2, SIGMA2)
        mu_dash2 = compute_mu_dash(mu2, RD_dash2, muj_list_B, RDj_list_B, Sj_list_B)
        c.execute("DELETE FROM user"+str(userA[0])+"_record")
        conn.commit()
    else: 
        SIGMA2=SIGMA_B
        mu_dash2=mu2; RD_dash2=RD2
    
    return ([S1,SIGMA1] + scale_up(mu_dash1, RD_dash1)), ([S2,SIGMA2] + (scale_up(mu_dash2, RD_dash2)))


def match_and_update_ratings_glicko(rounds): #3000명 큐 돌려서 잡힌 애들끼리 경쟁전 치르게함
    for i in range(rounds):
        ...
        for userA, userB in matches:
            ...
            c.execute("UPDATE users SET rating=?, RD=?, win=?, lose=?, draw=?, sigma=? WHERE id=?", (int(mu1), round(RD1,3), aWin, aLose, aDraw, round(sigma1,6), userA[0]))
            conn.commit()
            c.execute("UPDATE users SET rating=?, RD=?, win=?, lose=?, draw=?, sigma=? WHERE id=?", (int(mu2), round(RD2,3), bWin, bLose, bDraw, round(sigma2,6), userB[0]))
            conn.commit()
